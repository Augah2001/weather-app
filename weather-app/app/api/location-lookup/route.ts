...
export async function GET(request: NextRequest) {
    ...
    try {
        // Find location in DB by name
        const location = await prisma.location.findUnique({
            where: { name: locationName },
            select: { id: true, name: true, latitude: true, longitude: true }, // Select necessary fields
        });

        if (location) {
            console.log(`Location "${locationName}" found in DB.`);
            // Return the location details including coordinates
